...
class ZOOptimizer(nn_optimizer.NNOptimizer):
    ...
    def reset_state(self, keep_states=False, model=None, use_cuda=False, gpu_num=0):
        ...
        self.meta_model.copy_convex_model_from(model)
...
class ZOOptimizerAdam(nn_optimizer.NNOptimizer):
    ...
    def meta_update(self, model, data, target):
        ...
        self.v = self.beta2*self.v + (1-self.beta2)*g.pow(2)
        ...
        inputs = m_hat/(v_hat.sqrt() + self.eps)
...
class ZOOptimizerProp(nn_optimizer.NNOptimizer):
    ...
    def meta_update(self, model, data, target):
        ...
        m_hat = (self.m/(1-self.beta1_t)).view(-1)
...
# ZO optimizer with RNN Prop tricks (both UpdateRNN and QueryRNN)
class VarReducedZOOptimizerProp(nn_optimizer.NNOptimizer):

    def __init__(self, model, args, num_layers=1, input_dim=1, hidden_size=10, ckpt_path="", beta1=0.95, beta2=0.95, eps=1e-8):
        super(VarReducedZOOptimizerProp, self).__init__(model, args)
